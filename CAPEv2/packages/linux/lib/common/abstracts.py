...
class Package:
    ...
    def __init__(self, target, options={}, timeout=120, strace_ouput=None, **kwargs):
    ...
    def execute(self, path, arguments=""):
        ...
        @param arguments: executable arguments (optional string)
        @return: process pid
        """
        import shlex
        if arguments:
            cmd = shlex.split(f"{path} {arguments}")
        else:
