...
Operates in two modes:
  1. Offline / dataset mode  — reads a CSV, extracts all features, writes output CSV
  2. Live / pipeline mode    — called from mail_filter.py with an email message object

Three extraction tracks (mirrors the main pipeline):
  Track A — Semantic metadata (URLs, code, keywords, auth signals)
  Track B — Structural NLP-ready text (cleaned, tokenised)
  Track C — Attachment analysis (type classification + sandbox pipeline hook)
"""
from __future__ import annotations
import os, sys, re, json, html, hashlib, base64, logging
...
# ─── Attachment extension maps (mirrors sandbox_client) ───────────────────────
...
def extract_track_a(msg_or_row, body_text: str = "") -> dict:
    ...
    # ── URL features ──────────────────────────────────────────────────────────
    url_re    = re.compile(r'https?://(?:[-\w.]|(?:%[\da-fA-F]{2}))+(?:/[^\s<>"]*)?', re.I)
    href_re   = re.compile(r'href=["\']([^"\']+)["\']', re.I)
    all_urls  = url_re.findall(text)
    href_urls = href_re.findall(text)

    mismatch_re = re.compile(r'<a[^>]+href=["\']([^"\']+)["\'][^>]*>([^<]+)</a>', re.I)
    mismatches  = sum(
    ...
    # ── Phishing keywords ─────────────────────────────────────────────────────
    kw_counts = {f"keyword_{k.replace(' ','_')}": lower.count(k) for k in PHISHING_KEYWORDS}
    ...
    # ── Phone / IP counts ────────────────────────────────────────────────────
    features["phone_count"]      = len(re.findall(r'\b\d{3}[-.\s]?\d{3}[-.\s]?\d{4}\b', text))
    features["ip_address_count"] = len(re.findall(r'\b(?:\d{1,3}\.){3}\d{1,3}\b', text))

    return features


# ══════════════════════════════════════════════════════════════════════════════
# TRACK A (header section) — from live email message
# ══════════════════════════════════════════════════════════════════════════════
def extract_header_features(message) -> dict:
    ...
    features.update({
        ...
        "from_localpart_length":   len(from_clean.split('@')[0]) if '@' in from_clean else 0,
    ...
    features.update({
        "subject_text":         subject,         # raw text, passed to NLP
        ...
        "subject_has_suspended":int(bool(re.search(r'suspended|locked|closed', subj_low))),
        "subject_has_account":  int(bool(re.search(r'account|password|credential', subj_low))),
    ...
    if date_str:
        try:
            ...
            features["date_is_future"]  = int(date_obj > datetime.now(date_obj.tzinfo))
    ...
    features.update({
        ...
        "received_count_normalized": min(len(message.get_all("Received",[])) / 10.0, 1.0),
    })
...
# ══════════════════════════════════════════════════════════════════════════════
# TRACK C — Attachment feature extraction
# ══════════════════════════════════════════════════════════════════════════════
def extract_track_c(message) -> dict:
    """
    Walk email parts, extract attachment metadata, classify by type and OS target.
    Does NOT call sandbox directly (that happens in mail_filter via sandbox_client).
    Returns feature dict + sandbox_pipeline list for mail_filter to consume.
    """
    attachments       = []
    sandbox_pipeline  = []   # list of {filename, payload_b64, targets} for dispatch
    ...
    for part in message.walk():
        ...
        if payload:
            ...
            sandbox_pipeline.append({
                "filename":    filename or f"attachment_{att_count}",
...
def extract_features_from_email(message) -> dict:
    ...
    Returns:
      {
        "header":   { ...numeric header features + subject_text },
        "body_a":   { ...semantic body features },
        "body_b":   "clean text string",
        "track_c":  { ...attachment features + sandbox_pipeline list },
      }
    """
    # Extract raw body (HTML preserved for Track A)
    raw_body = ""
    try:
    ...
    # Merge header auth/structural into body_a for combined semantic_meta
    # (mail_filter uses a single semantic_meta for both models)
    semantic_meta = {**body_a_feats, **header_feats}

    return {
        "semantic_meta": semantic_meta,    # → header.py + body.py
        "clean_text":    clean_text,       # → body.py NLP
        "track_c":       track_c,          # → sandbox_client.py
    }
...
class PhishingEmailFeatureExtractor:
    """Extract comprehensive features from a CSV email dataset."""
    ...
    def load_dataset(self):
        for enc in ['utf-8','latin-1','iso-8859-1','cp1252']:
            try:
                ...
                logger.info(f"Loaded {self.csv_file} ({enc}) — {len(self.df)} rows")
                return self.df
    ...
    def _extract_row(self, row) -> dict:
        ...
        # Header fields from columns
        subject = str(row.get("subject", row.get("Subject",""))) if pd.notna(
            row.get("subject", row.get("Subject","--"))) else ""
        from_v  = str(row.get("from",  row.get("From","")))  if pd.notna(
            row.get("from",  row.get("From","--")))  else ""
        ...
        # Attachment columns (if dataset has them)
        for att_col in ["attachment","attachments","has_attachment"]:
            if att_col in row.index and pd.notna(row.get(att_col)):
                ...
                feats["attachment_count"] = int(bool(val and val not in ("0","false","none","")))
                break
        else:
    ...
    def run_pipeline(self, output_prefix: str = "phishing_features") -> pd.DataFrame:
        ...
        # Drop non-numeric columns for CSV output (keep clean_text, subject_text separately)
        ...
        logger.info(f"Saved {output_prefix}_features.csv  ({len(feats_num)} rows, "
                    f"{len(feats_num.columns)} features)")
        return feats
...
def main():
    import argparse
    ap = argparse.ArgumentParser(description="Extract features from phishing email dataset")
    ...
    print(f"Output: {args.output}_features.csv")
